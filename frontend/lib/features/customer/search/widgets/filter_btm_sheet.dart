...
const List<FilterOptions> filterOptionsList = [
...
class _FilterBtmSheetState extends State<FilterBtmSheet> {
  ...
  void initState() {
    ...
    _selectedPriceRange = List.generate(filterOptionsList.length, (_) => false);
  }
    ...
    if (_isPriceRangeSelected) {
      for (int i = 0; i < _selectedPriceRange.length; i++) {
        if (_selectedPriceRange[i]) {
          _minPrice = filterOptionsList[i].minValue;
          _maxPrice = filterOptionsList[i].maxValue;
        }
      }
    } else {
